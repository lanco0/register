...
module.exports = async (interaction) => {
  ...
  if (interaction.customId.split("_")[0] === "Whitelist") {
   ...
   let name = config.name.replace("-isim-", `${ic}`).replace("-ooc-", `${ooc}`).replace("-yaş-", `${yaş}`)
    ...
    await interaction.message.edit({ content: "Kullanıcıyı başarıyla kayıt ettin!", components: [new MessageActionRow({ components: interaction.message.components[0].components.map(x => x.setDisabled(true)) })] }).catch(err => {});
    ...
    let embed = new MessageEmbed()
      ...
      .setDescription(`> ${member.user} adlı kişi başarıyla kayıt oldu kişiye verdiğim roller = <@&${addRoles.join(">, <@&")}>`)
      .setFooter({ text: "Kayıdı gerçekleştiren yetkili: " + interaction.user.tag, iconURL: interaction.user.avatarURL({ dynamic: true }) })
      .setColor("64c4e1")
    
    bot.channels.cache.get(config.whitelistLog)?.send({ embeds: [embed], })
    
    interaction.followUp({
      content: `> ${emojiConfig.başarılı} **Başarılı!** Kullanıcıya başarıyla whitelist verildi.`,
      ephemeral: true,
    });
  }
